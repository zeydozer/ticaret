...
<div class="container">
    ...
    <div class="row user-login forms">
        ...
        <div class="col-md-6 offset-md-3 text-center">
            <h2>Şifremi {{ $token ? 'Sıfırla' : 'Unuttum' }}</h2>
            <form class="row" action="/token">
                ...
                <div class="col-xl-12">
                    <div class="form-line-no-line text-center">
                        <button class="m-auto">{{ $token ? 'SIFIRLA' : 'GÖNDER' }}</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    ...
    $(function()
    {
        ...
        @if (($path == 'giris' && isset($status)) || strpos($path, 'callback') !== false)

        uyari('<?php echo $status ?>', '<?php echo $message ?>');

        @if (isset($go)) setTimeout(function() { location.href = '{{ $go }}'; }, 4000); @endif
