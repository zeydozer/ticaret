...
<div class="container">
    ...
    <section class="row payment_successful">
        ...
        <div class="col-xl-12 status text-center {{ !$durum ? 'mb-0' : '' }}">
            <img src="/assets/images/{{ $durum ? 'basarili' : 'basarisiz' }}.svg">
            <span class="result">Ödeme {{ $durum ? 'Başarılı' : 'Başarısız' }}</span>
            <span class="result-text"><?php echo Session::get('mess'); Session::forget('mess'); ?></span>
            @if (!$durum) 
            <a href="/odeme">Tekrar Deneyin</a> 
            @elseif (Cookie::has('uye')) 
