    ...
    <section class="basket">
        ...
        <div class="basket-table">
            <div class="basket-tt">
                ...
                @foreach ($sepettekiler as $i => $sepet)
                ...
                <div class="row table-line @if (!$urun->stok) bg-light @endif"
                ...
                title="Ürün stokta yok.." <?php endif ?>>
                    <div class="col-xl-6 col-lg-6 col-md-12 table-inner-line sepet-isim">
                        ...
                        <a href="/urun/{{ $urun->url }}">
                            <i class="fa fa-trash" data="{{ $sepet->id }}"></i>
                            <img src="/img/{{ $profil }}" alt="{{ $urun->isim }}"
                            @if ($profil == 'logo.png') style="opacity: .5" @endif>
                            @if ($urun->paket)
                            <b>{{ \App\Urun::find($urun->set_id)->isim }} - </b>
                            @endif
                            <span>{{ $urun->isim }}</span>
                        </a>
                    </div>
                    <div class="col-xl-2 col-lg-2 col-md-4 col-6 table-inner-line 
                    table-select-amount" data="{{ $sepet->id }}">
                        <span class="mobile-title">ÜRÜN ADEDİ</span>
                        <span class="input-group-btn">
                            <button type="button" class="btn btn-default btn-number" 
                            data-type="minus" data-field="adet[{{ $i }}]" sepet="--">
                                <span class="glyphicon glyphicon-minus"></span>
                            </button>
                        </span>
                        <input type="text" name="adet[{{ $i }}]" class="form-control input-number" 
                        value="{{ $sepet->adet }}" min="1" max="{{ $urun->stok ? $urun->stok : $sepet->adet }}">
                        <span class="input-group-btn">
                            <button type="button" class="btn btn-default btn-number" 
                            data-type="plus" data-field="adet[{{ $i }}]" sepet="++">
                    ...
                    <div class="col-xl-2 col-lg-2 col-md-4 col-6 table-inner-line">
                        ...
                        <span class="price">{{ number_format($fiyat, 2, ',', '.') }} TL</span>
                    </div>
                    <div class="col-xl-2 col-lg-2 col-md-4 table-inner-line">
                        <span class="mobile-title">TOPLAM</span>
                        <span class="price total">{{ number_format($fiyat * $sepet->adet, 2, ',', '.') }} TL</span>
                    </div>
                </div>
                @endforeach
                ...
                $toplam_k = $hasDiscount ? $indirimli : $toplam;

                $kargo = $toplam_k >= $limit ? 0 : \DB::table('ayar')->where('tip', 'kargo')->first()->data;
                ...
                <div class="row table-line total-row cargo">
                    <div class="col-xl-6 col-lg-6 col-md-12 table-inner-line text-right">
                        <span class="mobile-title mb-lg-3">KARGO</span>
                        <img src="/img/logo.png" class="invisible">
                        <a href="#" onclick="return false"><b>KARGO</b></a>
                    </div>                    
                    <div class="col-xl-4 col-lg-4 col-md-12 table-inner-line prices offset-xl-2 offset-lg-2 @if ($toplam_k >= $limit) free-text @endif">
                        <p style="@if ($toplam_k < $limit) display: none @endif">{{ $limit }} TL ve üzeri</p>
                        <span style="font-weight: 800; @if ($toplam_k < $limit) display: none @endif">Kargo BEDAVA</span>
                        <span class="price total" style="@if ($toplam_k >= $limit) line-height: 40px @endif">{{ number_format($kargo, 2, ',', '.') }} TL</span>
                    </div>
                </div>
                
                <div class="row table-line total-row amount">
                    <div class="col-xl-6 col-lg-6 col-md-12 table-inner-line text-right">
                        <span class="mobile-title mb-lg-3">TOPLAM</span>
                        <img src="/img/logo.png" class="invisible">
                        <a href="#" onclick="return false"><b>TOPLAM</b></a>
                    </div>
                    <div class="col-xl-2 col-lg-2 col-md-12 col-12 table-inner-line table-select-amount">
                        <span class="mobile-title">ADET</span>
                        <input type="text" name="adet" class="form-control input-number" value="{{ $adet }}" readonly>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-12 table-inner-line prices @if ($hasDiscount) free-text @endif">
                    	<div class="has-discount" @if (!$hasDiscount) style="display: none" @endif>
                            <p>İlk alışverişinize özel</p>
                            <span style="font-weight: 800"> <text>{{ $hasDiscount ? $indirim['b'] : null }}</text> TL indirim kazandınız! </span>
                            <span class="old" style="font-weight: 800; margin-top: 10px">{{ $hasDiscount ? number_format($indirimsiz, 2, ',', '.') : null }} TL</span>
                        </div>
                        <span class="price total" @if ($hasDiscount) style="line-height: 20px" @endif>{{ number_format($hasDiscount ? $indirimli : $toplam, 2, ',', '.') }} TL</span>
                    </div>
                </div>
            </div>
    ...
    $(function()
    {
        ...
        $('.input-number').keyup(function ()
        {
            ...
            $.post('/sepet', datas, function(resp)
            {
                ...
                if (resp.indirimli != undefined)
                {
                    $('.total-row.amount .old').html(resp.indirimsiz +' TL');

                    $('.total-row.amount .total').html(resp.indirimli +' TL').css('line-height', '20px');

                    $('.has-discount span:eq(0) text').html(resp.indirim_b);

                    $('.has-discount').show().closest('div').addClass('free-text');
                }

            	else
                {
                    $('.total-row.amount .total').html(resp.fiyat +' TL').removeAttr('style');

                    $('.has-discount').hide().closest('div').removeClass('free-text');
                }

                $('.total-row.cargo .total').html(resp.kargo +' TL');

                resp.kargo = resp.kargo.replace('.', '').replace(',', '.');

                if (resp.kargo == 0)
                {
                    $('.total-row.cargo p, .total-row.cargo .col-xl-4 span:eq(0)').show();

                    $('.total-row.cargo p').closest('div').addClass('free-text');

                    $('.total-row.cargo .total').css('line-height', '40px');
                }

                else 
                {
                    $('.total-row.cargo p, .total-row.cargo .col-xl-4 span:eq(0)').hide();

                    $('.total-row.cargo p').closest('div').removeClass('free-text');

                    $('.total-row.cargo .total').removeAttr('style');
                }
