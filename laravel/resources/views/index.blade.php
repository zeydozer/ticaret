...
<head>
    ...
    @if (strpos(Request::path(), 'urun/') !== false)
    @php
    ...
    @endphp
    <meta name="keywords" content="{{ $urun->keyword }}" />
    @else
    ...
    <title>@yield('title') | {{ $seo['author'] }}</title>
    ...
    <meta property="og:url" content="{{ Request::url() }}?id={{ Request::get('id') }}" />
    <meta property="twitter:site" content="{{ Request::url() }}?id={{ Request::get('id') }}" />
    <meta property="og:title" content="{{ strip_tags($urun->isim) }}" />
    <meta property="twitter:title" content="{{ strip_tags($urun->isim) }}" />
    <meta property="og:image" content="{{ url('img/'. $profil) }}" />
    <meta property="twitter:image" content="{{ url('img/'. $profil) }}" />

    @if ($urun->ozellik)

    <meta property="og:description" content="{{ strip_tags(implode(' || ', json_decode($urun->ozellik))) }}" />
	<meta property="twitter:description" content="{{ strip_tags(implode(' || ', json_decode($urun->ozellik))) }}" />
...
<body>
    ...
    <header>
        ...
        <div class="nav-campaign" style="background: #FF0000">
            <div class="container">
                <span>
                    <strong>
                        Şimdi üye olun, ilk alışverişinize özel {{ $indirim['a'] }} TL'ye
                        {{ $indirim['b'] }} TL kazanın!
                    </strong>
                </span>
            </div>
        </div>
        ...
        <div class="nav-campaign">
            <div class="container">
                <span>
                    <strong>{{ $kargo }} TL ve üzeri alışverişlerinizde Kargo BEDAVA</strong>
                </span>
            </div>
        </div>
        @endif
            ...
            <div class="container">
                <div class="row">
                    <a class="navbar-brand" href="/">
                        <img src="/img/logo.png" alt="{{ $seo['author'] }}">
                    </a>
                        ...
                        <div class="search-top">
                            <ul class="list-inline">
                                ...
                                <li class="list-inline-item">
                                    <a href="/urunler?ara={{ $seo['arama_'. $i] }}">{{ $seo['arama_'. $i] }}</a>
                                </li>
                                @endfor
                            </ul>
                        </div>
                            ...
                            <li class="list-inline-item">
                                <a href="/sepet" title="">
                                    <span class="icon--top">
                                        ...
                                        <span class="piece sepet-adet">{{ $sepet_adet }}</span>
                                    </span>
                                </a>
                            </li>
                        </ul>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                                    ...
                                    <div class="col-md-8">
                                        <ul class="list-unstyled">
                                            ...
                                            <li class="border-top" style="padding-top: 10px">
                                                <a href="/urunler/{{ $kategori->url }}">
                                                    <!-- <img src="/img/{{ $kategori->foto }}"> -->
                                                    {{ $kategori->isim }}
                                                </a>
                                            </li>
                                            @php $altlar = \App\Http\Controllers\Controller::kat_getir($kategori->id, $kategori->isim, true) @endphp
                                            @if (count($altlar) > 0)
                                            @foreach ($altlar as $id => $isim)
                                            @php $kategori = \App\Kategori::find($id) @endphp
                                            <li>
                                                <a href="/urunler/{{ $kategori->url }}" class="font-weight-normal">
                                                    <!-- <img src="/img/{{ $kategori->foto }}"> -->
                                                    {{ $kategori->isim }}
                                                </a>
                                            </li>
                                    ...
                                    <div class="col-md-4 mt-3 mt-md-0">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-3 col-md-6 col-4">
                                                <a href="/urunler?marka[]={{ $marka->name }}" class="font-weight-bold">
                                                    <img src="//entegre.ruberu.com.tr/assets/images/brands/{{ $marka->photo }}">
                                                    <!-- Bütün Ürünler -->
                                                </a>
                                            </div>
                                            @endforeach
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item mr-0">
                            <a href="//wa.me/9{{ str_replace(' ', '', $iletisim['whatsapp']) }}?text=Merhaba" class="nav-link">
                ...
                <nav id="ml-menu" class="menu">
                    ...
                    <div class="menu__wrap">
                        ...
                        <ul data-menu="submenu-all" id="submenu-all" class="menu__level" tabindex="-1" role="menu" aria-label="Hepsi">
                            ...
                            @php $altlar[$kategori->id] = \App\Http\Controllers\Controller::kat_getir($kategori->id, $kategori->isim, true) @endphp

                            <li class="menu__item" role="menuitem">
                                <a class="menu__link" href="/urunler/{{ $kategori->url }}" 
                                @if (count($altlar[$kategori->id]) > 0) data-submenu="submenu-{{ $kategori->id }}" aria-owns="submenu-{{ $kategori->id }}" @endif>
                                    {{ $kategori->isim }}
                                </a>
                            </li>                            
                            @endforeach
                        </ul>
                        @foreach ($altlar as $id => $kategoriler)
                        @if (count($kategoriler) > 0)

                        @php $kategori = \App\Kategori::find($id) @endphp

                        <ul data-menu="submenu-{{ $id }}" id="submenu-{{ $id }}" class="menu__level" tabindex="-1" role="menu" aria-label="{{ $kategori->isim }}">
                            @foreach ($kategoriler as $id => $isim)
                            
                            @php $kategori = \App\Kategori::find($id) @endphp
                            
                            <li class="menu__item" role="menuitem">
                                <a href="/urunler/{{ $kategori->url }}" class="menu__link">
                                    <!-- <img src="/img/{{ $kategori->foto }}"> -->
                                    {{ $kategori->isim }}
                                </a>
                            </li>
                        ...
                        <ul data-menu="submenu-brand" id="submenu-brand" class="menu__level" tabindex="-1" role="menu" aria-label="Markalar">
                            @foreach ($markalar as $marka)
                            <li class="menu__item" role="menuitem">
                                <a class="menu__link" href="/urunler?marka[]={{ $marka->name }}">
                                    <img src="//entegre.ruberu.com.tr/assets/images/brands/{{ $marka->photo }}">
                                    {{ $marka->name }}
                                </a>
                            </li>
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-3 col-lg-3 col-6">
                                <ul class="list-unstyled">
                                    ...
                                    @for ($i = 1; $i <= 2; $i++)
                                    <li><a href="/urunler?ara={{ $seo['arama_'. $i] }}">{{ $seo['arama_'. $i] }}</a></li>
                                    @endfor
                                </ul>
                            </div>
                            <div class="col-xl-3 col-lg-3 col-6">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-xl-10 m-auto d-lg-flex">
                        ...
                        <div class="col-lg-4 px-lg-0 text-right"> Copyright &copy; {{ date('Y') }}</a></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        gtag('event', 'conversion', 
        {
            ...
            'value': Number('1.0'),
            'currency': 'TRY',
            'transaction_id': '{{ Session::get("conversion") }}'
        });
