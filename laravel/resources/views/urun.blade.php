...
<div class="container">
    <div class="row breadcrumb-row align-items-center my-2">
        ...
        @if (($urun->indirim && $urun->indirim >= $limit) || ($urun->fiyat >= $limit))
    ...
    <div class="row mb-lg-5 mb-md-4 mb-5">
        <div class="col-xl-6 col-lg-5 col-md-4">
            <div class="detail-slide">
                @if (count($fotolar) > 0 || strpos($urun->video, '/watch?v=') !== false)
                @if (count($fotolar) > 0)
                @foreach ($fotolar as $foto)
                <div class="">
                    <img src="/img/{{ $foto->deger }}" class="img-fluid" alt="{{ $urun->isim }}">
                </div>
                @endforeach
                @endif
                @if (strpos($urun->video, '/watch?v=') !== false)
                <div class="">
                    
                    <?php $temp = explode('/watch?v=', $urun->video) ?>                    
                    
                    <iframe src="//www.youtube.com/embed/{{ $temp[1] }}?rel=0&showinfo=0" allowfullscreen></iframe>
                </div>
                @endif
                @else
                <div class="">
                    <img src="/img/logo.png" class="img-fluid" alt="{{ $urun->isim }}"
                ...
                <div class="col-10 m-auto">
                    <div class="row justify-content-center">
                        ...
                        <div class="col-lg-2 col-md-4 col-3 px-0">
                            <a href="/img/{{ $foto->deger }}" data-fancybox="gallery">
                                <img src="/img/{{ $foto->deger }}" class="img-fluid" alt="{{ $urun->isim }}">
                            </a>
                        </div>
                        @endforeach
                        ...
                        @if (strpos($urun->video, '/watch?v=') !== false)
                        <div class="col-lg-2 col-md-4 col-3 px-0">
                            <a href="//www.youtube.com/embed/{{ $temp[1] }}?rel=0&showinfo=0" data-fancybox="gallery">
                                <img src="//img.youtube.com/vi/{{ $temp[1] }}/hqdefault.jpg" class="img-fluid" alt="{{ $urun->isim }}">
                            </a>
                        </div>
                        @endif
                        @else
        ...
        <div class="col-xl-6 col-lg-7 col-md-8 detail-content mb-0">
          <!-- <h1>{{ $urun->isim }}</h1> -->
          @if ($urun->marka)
          <a href="/urunler?marka[]={{ $urun->marka }}" class="brand">{{ $urun->marka }}</a>
          @endif
            <h1>
                {{ $urun->isim }}
                <!-- @if ($urun->kod)
            ...
            <span class="winnings">Kazancınız %{{ intval($oran) }}</span>
            @else
            <div class="prices d-flex align-items-center">
            ...
            @if (!$urun->set)
                @if (!$urun->paket)
                    @if ($urun->stok > 0)
                        <form class="add-basket d-flex">
                            <div class="basket-select-amount">
                                ...
                                <input type="text" name="adet" class="form-control input-number" value="1" min="1" max="{{ $urun->stok }}">
                                <span class="input-group-btn">
                    ...
                    <div class="d-flex flex-wrap mb-3" style="margin-left: -.5rem; margin-right: -.5rem">
                        ...
                        <a href="/urun/{{ $variant->url }}" class="d-block mx-2 mt-1">
                            <img src="/img/{{ $variant->profil }}" class="img-fluid" alt="{{ $variant->isim }}"
            ...
            @else
                <div class="py-2"></div>
                @foreach (json_decode($urun->set) as $i => $urun_id)

                <?php $urun_s = \App\Urun::find($urun_id) ?>

                <form class="add-basket row mx-0 align-items-center py-1" data="{{ $urun_s->id }}">
                    <div class="product-name col-md-7 col-12">
                        <a href="/urun/{{ $urun_s->url }}">{{ $urun_s->isim }}</a>
                    </div>
                    <div class="basket-select-amount col-md-2 col-6 px-0">
                        <span class="input-group-btn">
                            <button type="button" class="btn btn-default btn-number" data-type="minus" data-field="adet-{{ $i }}">
                                <span class="glyphicon glyphicon-minus"></span>
                            </button>
                        </span>
                        <input type="text" name="adet-{{ $i }}" class="form-control input-number w-100"
                        <?php if ($urun_s->stok) : ?> value="1" min="1" max="{{ $urun_s->stok }}" <?php else : ?>
                        value="0" min="0" max="0" disabled <?php endif ?>>
                        <input type="hidden" name="adet" value="{{ $urun_s->stok ? 1 : 0 }}">
                        <span class="input-group-btn">
                            <button type="button" class="btn btn-default btn-number" data-type="plus" data-field="adet-{{ $i }}">
                                <span class="glyphicon glyphicon-plus"></span>
                            </button>
                        </span>
                    </div>
                    <button class="add-basket-button col-md-3 col-6" disabled>
                        {{ number_format($urun_s->indirim > 0 ? $urun_s->indirim : $urun_s->fiyat, 2, '.', ',') }} TL
                    </button>
                </form>
                @endforeach
                <div class="py-3">
            ...
            @if ($urun->ozellik)
            <ul class="inf">
                @foreach (json_decode($urun->ozellik) as $ozellik)
                <li>{{ $ozellik }}</li>
                @endforeach
            </ul>
            @endif
            <div class="share-products @if (!$urun->ozellik) mt-0 @endif">
    ...
    <div class="row sub-features mb-lg-5 mb-md-4 text-center">
        <div class="col-md-2 col-6">
            <div class="features-item">
              <img src="/assets/images/icb1.svg" alt="{{ $limit }} TL ÜSTÜ ÜCRETSİZ KARGO">
              <h4>{{ $limit }} TL ÜSTÜ ÜCRETSİZ KARGO</h4>
            </div>
        </div>
        <div class="col-md-2 col-6">
            <div class="features-item">
...
<div class="container detail-tabs-content fade show" id="tab-1">
    <div class="detail-tab-content">
        ...
        <div class="row">
            <div class="col-12">
                <ul class="mb-0">
                    @foreach (json_decode($urun->teknik) as $isim => $aciklama)
                    <li>
                        <span class="title">{{ $isim }}:</span>
                        <span class="list-content">{{ $aciklama }}</span>
                    </li>
                    @endforeach
                </ul>
            </div>
        ...
        @if ($urun->foto)
        @foreach (json_decode($urun->foto) as $foto)
        <div class="image-content">
            <img src="/img/{{ $foto }}" class="img-fluid" alt="{{ $urun->isim }}">
        </div>
        @endforeach
        @endif

        @if (!$urun->aciklama && !$urun->teknik && !$urun->foto)
        <div class="row">
            <div class="col-12">
...
<div class="container detail-tabs-content fade" id="tab-2">
    <div class="detail-tab-content">
        ...
        <iframe src="https://www.youtube.com/embed/{{ $temp[1] }}?rel=0&showinfo=0" allowfullscreen></iframe>
    </div>
</div>
@endif
...
<div class="container detail-tabs-content fade" id="tab-3">
    <div class="detail-tab-content">
        ...
        <div class="row mt-{{ $i == 0 ? 5 : 3 }} yorum">
            <div class="col-xl-8 col-lg-8 m-auto">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <b>{{ $yorum->isim }}</b><br>
                        <small class="d-block mt-2">{{ date('d/m/Y - H:i', strtotime($yorum->tarih)) }}</small>
                    </div>
                    <div class="col-md-4">
                        <div class="stars m-auto">
                            <div style="overflow: hidden; width: {{ $yorum->puan * 20 }}%">
                                <img src="/img/stars-checked.png" height="20">
                            </div>
                        </div>
                    </div>
                </div>
                <p class="mb-0 mt-3">{{ $yorum->yorum }}</p>
            </div>
        </div>
        @endforeach
...
<div class="container detail-tabs-content fade taksit" id="tab-5">
    ...
    <i class="fas fa-exclamation-circle mr-1"></i> {{ $installmentInfo->getErrorMessage() }}
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <img src="//sandbox-merchant.iyzipay.com/static/media/{{ $bank_logo[$i] }}.svg" alt="{{ $installmentDetails->bankName }}">
            <table class="table-sm" width="100%">
                ...
                <tr>
                    <td>{{ $installmentPrices->installmentNumber }}</td>
                    <td>{{ $installmentPrices->installmentPrice }}</td>
                    <td>{{ $installmentPrices->totalPrice }}</td>
                </tr>
...
<div class="container">
    ...
    <section class="row products">
        ...
        @foreach ($bakanlar as $urun_d)
        <div class="col-xl-3 col-lg-3 col-md-6 col-6">
            <a href="/urun/{{ $urun_d->url }}">
                <div class="product-container">
                    ...
                    <img src="/img/{{ $profil }}" class="img-fluid" alt="{{ $urun_d->isim }}"
                    @if ($profil == 'logo.png') style="opacity: .5" @endif>
                    <h5>
                        {{ $urun_d->isim }}
                        <!-- @if ($urun_d->kod)
                        <small>{{ $urun_d->kod }}</small>
                        @endif -->
                    </h5>
                    <div class="prices text-center">
                        @if ($urun_d->indirim > 0)
                        <span class="old">{{ number_format($urun_d->fiyat, 2, ',', '.') }} TL</span>
                        <span class="product-price">{{ number_format($urun_d->indirim, 2, ',', '.') }} TL</span>
                        @else
                        <span class="old invisible">{{ number_format($urun_d->fiyat, 2, ',', '.') }} TL</span>
                        <span class="product-price">{{ number_format($urun_d->fiyat, 2, ',', '.') }} TL</span>
                        @endif
                    </div>
                    @if ($urun_d->indirim > 0)
                    <span class="winnings">Kazancınız %{{ intval($urun_d->oran) }}</span>
                    @else
                    <span class="winnings invisible">Kazancınız %{{ intval($urun_d->oran) }}</span>
                    @endif
                </div>
            </a>
            <div class="hover">
                @if ($urun_d->stok && !$urun_d->paket)
                <a href="{{ $urun_d->id }}" class="add-basket-d">Sepete Ekle</a>
                @endif
                <a href="/urun/{{ $urun_d->url }}">İncele</a>
            </div>
        </div>
        @endforeach
    </section>
    @endif
    @if (isset($digerler) && $urun->kat_id)
    @if (count($digerler) > 0)
    <section class="row products">
        <div class="box-title">
            <h2>BU KATEGORİDEKİ DİĞER ÜRÜNLER</h2>
        </div>
        @foreach ($digerler as $urun_d)
        <div class="col-xl-3 col-lg-3 col-md-6 col-6">
            <a href="/urun/{{ $urun_d->url }}">
                <div class="product-container">
...
<script>
    ...
    $(function()
    {
        ...
        $('.add-basket').submit(function(e)
        {
            ...
            $.ajax(
            {
                ...
                success: function(resp)
                {
                    ...
                    $('#sepet-sonuc .modal-body span').text('{{ $urun->isim }}');
        ...
        $(".input-number").change(function()
        {
            ...
            var fiyat = parseFloat('{{ $urun->indirim }}');
                eski = parseFloat('{{ $urun->fiyat }}');

            @else

            var fiyat = parseFloat('{{ $urun->fiyat }}');
        ...
        $('#yorum-form').submit(function(e)
        {
            ...
            form_data.append('urun_id', '{{ $urun->id }}');
            form_data.append('_token', '{{ csrf_token() }}');
        ...
        $('.share-products a').click(function(e)
        {
            ...
            window.open(links[social] +'{{ urlencode(Request::url() ."?id=") }}'+ id, '', 'width=600, height=400');
        });
    });
